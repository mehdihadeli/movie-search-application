...
public class MoviesController : BaseController
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [SwaggerOperation(
        Summary = "Get specific movie by id with its trailers",
        Description = "Get specific movie by id with its trailers"
    )]
    public async Task<ActionResult> GetWithTrailersById(
        [FromRoute] int id,
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [SwaggerOperation(
        Summary = "Get specific movie by imdbId with its trailers.",
        Description = "Get specific movie by imdbId with its trailers."
    )]
    public async Task<ActionResult> GetWithTrailersByImdbId(
    ...
    public async Task<ActionResult> SearchByTitleAsync(
        [FromQuery] SearchMoviesByTitleRequest request,
        CancellationToken cancellationToken
    )
    {
        var query = new SearchMovieByTitleQuery { Page = request.Page, SearchKeywords = request.SearchKeywords };
        var result = await Mediator.Send(query, cancellationToken);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [SwaggerOperation(
        Summary = "Search movies by different parameters",
    ...
    {
        var query = new SearchMovieQuery(
            request.SearchKeywords,
            request.Page,
            request.Year,
            request.PrimaryReleaseYear,
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [SwaggerOperation(
        Summary = "Get a list of upcoming movies in theatres",
        Description = "Get a list of upcoming movies in theatres"
    )]
    public async Task<ActionResult> GetUpcomingMoviesAsync(
        [FromQuery] GetUpcomingMoviesRequest request,
        CancellationToken cancellationToken
    )
    {
    ...
    public async Task<ActionResult> GetPopularMoviesAsync(
        [FromQuery] GetPopularMoviesRequest request,
        CancellationToken cancellationToken
    )
    {
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [SwaggerOperation(
        Summary = "Get the cast and crew for a movie.",
        Description = "Get the cast and crew for a movie."
    )]
    public async Task<ActionResult> GetMovieCreditsAsync(
        [FromQuery] GetMovieCreditsRequest request,
        CancellationToken cancellationToken
    )
    {
